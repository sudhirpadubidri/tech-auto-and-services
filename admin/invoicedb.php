...
$conn=mysqli_connect("localhost:3308","root","","invoicedb");
if(!$conn){
...
function sendMail($pdf,$email)
    {
        ...
        try {
            ...
            $mail->Username   = 'techautoandservices@gmail.com';                     //SMTP username
            $mail->Password   = 'bmdwzuclgprxmejx';                               //SMTP password
            $mail->SMTPSecure = PHPMailer::ENCRYPTION_SMTPS;            //Enable implicit TLS encryption
            $mail->Port       = 465;                                    //TCP port to connect to; use 587 if you have set `SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS`
        
            //Recipients
            $mail->setFrom('techautoandservices@gmail.com', 'Tech Auto and Services');
...
if(isset($_POST['submit']))
    {
        ...
        if($res)
            ...
            if(mysqli_num_rows($res1)>0)
                {
                    $html='<div style="border-top:1px solid black;border-bottom:1px solid black;"><center><h2 style="margin-left:35%;">Tech Auto & Services</h2></center><h5 style="margin-left:36%;">One stop for all your vehicle needs!</h5></div><br>';
                        while($row=mysqli_fetch_assoc($res1))
                            {
